...
async function react(r, u, H = "") {
  try {
    ...
    console.log("🚨 *Error in Asta.js/react()* 🤡\n", z);
  }
}

let note = {
  info: "⚠️ *Listen up, brainless!* You better provide the first parameter as `{user: botNumber}` and the second as `note text | id`. If you mess it up, don’t cry. 😏"
};
...
note.delnote = async (r, u) => {
  try {
    ...
    let o = H.notes;
    let z = "🤡 *Oi dummy, provide a valid note ID!*";
    ...
    if (o[u]) {
      ...
      z = "🗑️ *Note with ID: " + u + " has been wiped out of existence!* 🔥";
    }
    ...
    return {
      status: false,
      error: h,
      msg: "🚫 *Tch, I can't delete this trash note because of an error. Try again later, fool!* 😏"
    };
  }
};
note.delallnote = async (r, u = "") => {
  try {
    ...
    return {
      status: true,
      msg: "🔥 *All your useless notes have been wiped from existence!* 🚮"
    };
  } catch (H) {
    console.log("note.delnote ERROR : ", H);
    return {
      status: false,
      error: H,
      msg: "❌ *Request failed. Maybe try again when you're smarter?* 😏"
    };
  }
};
note.allnotes = async (r, u = "") => {
  try {
    ...
    let o = H.notes;
    let z = "⚠️ *Are you dumb? Provide a valid note ID!* 🤡";
    ...
    console.log("❌ *note.allnotes ERROR:* ", f);
    return {
      status: false,
      error: f,
      msg: "😡 *I can't delete notes because of your dumb error!*"
    };
  }
};
  ...
  try {
    ...
    if (u && u.includes(":")) {
      ...
      if (K.toLowerCase() === "richel" || K === "1") {
        ...
        H = 34;
      } else if (K.toLowerCase() === "VRT" || K === "36") {
        H = 35;
